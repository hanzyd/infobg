...
def _process_one_year(file_name: str, session: Session, strict=False) -> list:
    ...
    for num, line in enumerate(lines):
        ...
        if not d_index:
            print(f'{file_name}:{num} Не намирам област {dist_name} община {mun_name} селище {town_name}')
            continue
        ...
        if not s_index:
            print(f'{file_name}:{num:4} Не намирам селище {town_name} в област {dist_name} и община {mun_name}')
            continue
